...
export const CreateOrder = () => {
  ...
  const getOptionsProducts = useCallback(
    debounce(async (searchWord) => {
      ...
      try {
        const { data, status } =
          await productForPurchaseOrderServices.searchProduct(searchWord);
        if (status === ResponseStatusEnum.OK) {
  ...
  const addItemToTable = async () => {
    if (selectedItem) {
      try {
        ...
        const { data } = await productForPurchaseOrderServices.getProductId(
          selectedItem.value
        );
        ...
        if (!data) {
          AlertComponent.error(
            "Error",
            "El producto no tiene valor total configurado!"
          );
          return;
        }
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      const { data, status } =
        await productForPurchaseOrderServices.saveOrderProducts(
          dataToSend,
          params.id
        );
  ...
  const calculateItemsWithTotal = () => {
    return items
      ...
      .map((item) => {
        ...
        const totalByItem = Math.ceil(
          item.valor_unitario * item.quantity * (1 - discountRate / 100)
        );
        return {
          producto: item.id,
    ...
    <>
      <div className="main-container">
        <HeaderImage
          ...
          bannerInformation={
            "Todo está listo para que completes tu pedido de forma rápida y sencilla."
          }
            ...
            <Row className="justify-content-center">
              <Col md={6} className="d-flex align-items-center">
                <div className="select-container d-flex align-items-center">
                  <Select
                    className="gov-select flex-grow-1" // Agrega flex-grow-1 para que ocupe el espacio disponible
                      ...
                      <Form.Control
                        ...
                        onChange={(e) =>
                          handleDiscountChange(
                            index,
                            limitToFourDecimals(e.target.value)
                          )
                        }
                      />
                    </td>
