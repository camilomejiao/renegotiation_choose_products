...
const IMAGE_SLOTS = new Set([
  UploadFileEnum.EVIDENCE1,
  UploadFileEnum.EVIDENCE2,
]);
 ...
 * @param {Object} row - Fila del DataGrid.
 */
const hasPdfConsolidado = (row) =>
  Boolean(row?.evidencePdf?.consolidatedFileUrl);
...
const hasPdfOK = (row) =>
  Boolean(
    row?.evidencePdf?.consolidatedFileUrl &&
      row?.evidencePdf?.feFileUrl &&
      row?.evidenceImg?.imgEvidence1Url &&
      row?.evidenceImg?.imgEvidence2Url
  );
...
export const Deliveries = () => {
  ...
  const [
    ...
    setIsReadyToPrintDeliveryInformation,
  ] = useState(false);
  ...
  const isButtonDisabled = (row, rolId = userAuth?.rol_id) => {
    if (
      (row.actions.approvedTerritorial === true ||
          ...
          prevRows.map((row) =>
            row.id === rowId
              ? {
                  ...row,
                  fileName: file.name,
                  evidencePdf: URL.createObjectURL(file),
                }
              : row
          )
        );
  ...
  const handleViewFile = async (pdfUrl) => {
    if (!pdfUrl) {
      AlertComponent.error(
        "Error",
        "No hay un archivo cargado para este producto."
      );
      return;
    }
  ...
  const handleCreateDeliveries = async () => {
    ...
    const dataSupplier = selectedSupplier
      ? selectedSupplier.value
      : suppliers[0].id;

    setLoading(true);
    try {
      const { data, status } = await deliveriesServices.productsToBeDelivered(
        dataSupplier,
        params.id
      );
  ...
  const handleApproveByAudit = async () => {
    if (!action || comment.trim() === "") {
      showError(
        "Validación",
        "Debes seleccionar una acción o escribir un comentario."
      );
      return;
    }
    ...
    try {
      ...
      const { status } = await deliveriesServices.approveDelivery(
        selectedDeliveryId,
        payload
      );
      if (status === ResponseStatusEnum.OK) {
        ...
        showAlert(
          "Bien hecho!",
          `Entrega ${
            action === "approve" ? "Aprobada" : "Denegada"
          } exitosamente!`
        );
  ...
  const handleFileChange = async (file, deliveryId, fileName, numeroFE) => {
    if (file) {
      ...
      try {
        const { status } = await deliveriesServices.evidenceOfDeliveries(
          deliveryId,
          formData
        );
  ...
  const handleSaveProduct = async () => {
    try {
      ...
      const { data, status } = await deliveriesServices.saveProducts(
        dataSupplier,
        params.id,
        dataSaveProducts
      );
