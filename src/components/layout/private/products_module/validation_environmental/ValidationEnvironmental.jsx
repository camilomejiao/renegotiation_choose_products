...
export const ValidationEnvironmental = () => {
  ...
  const [environmentalCategoriesColumns, setEnvironmentalCategoriesColumns] =
    useState([]);
  ...
  const getConvocations = async () => {
    ...
    try {
      const { data, status } =
        await convocationProductsServices.getConvocations();
  ...
  const getEnvironmentalColumns = async () => {
    const columns = await getEnvironmentalCategoriesColumns(
      handleSelectChange,
      handleCustomChange
    );
  ...
  const createApprovalPayload = (idsBatch, estado, comentario) => ({
    ...
    comentario: comentario
      ? comentario
      : "Productos aprobados exitosamente por ambiental!",
  });
  ...
  const sendBatchApproval = async (payload) => {
    try {
      const { status } =
        await convocationProductsServices.approveOrDennyEnvironmental(payload);
      if (status === ResponseStatusEnum.OK) {
  ...
  const buildData = (product, keys) => {
    return Object.fromEntries(
      keys.map((key) => [key, parseInt(product[key]) || 0])
    );
  };
    ...
    try {
      ...
      if (editedProducts.length === 0) {
        AlertComponent.warning(
          "",
          "No hay productos modificados para guardar."
        );
        return;
      }
   ...
   */
  const showInfo = (message, subtitle = "") =>
    AlertComponent.info(subtitle, message);
