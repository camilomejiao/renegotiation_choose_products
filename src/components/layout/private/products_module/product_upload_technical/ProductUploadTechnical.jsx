...
export const ProductUploadTechnical = () => {
  ...
  const handleSaveProducts = async () => {
    ...
    try {
      ...
      const { data, status } =
        await convocationProductsServices.saveProductsByConvocation(sendData);
  ...
  useEffect(() => {
    const clipboardListener = async (event) => {
      if (!event.clipboardData) return;

      const clipboardData = event.clipboardData.getData("text");
      const separator = clipboardData.includes("\t")
        ? "\t"
      ...
      if (!separator) {
        handleError(
          "Error",
          "No se detectó un separador válido en los datos pegados, debe ser tabulación o coma."
        );
        return;
      }
      ...
      const newRows = parsedData.map((rowData) => {
        const rowObject = {};
        ...
        baseColumns.forEach((col, colIndex) => {
          if (col.field === "unit") {
            const matchedUnit = units.find(
              (option) =>
                option.nombre.trim().toLowerCase() ===
                rowData[colIndex]?.trim().toLowerCase()
            );
