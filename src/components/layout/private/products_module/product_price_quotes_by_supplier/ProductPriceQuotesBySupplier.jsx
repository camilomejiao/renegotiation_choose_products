...
export const ProductPriceQuotesBySupplier = () => {
  ...
  const extractObservations = useCallback((rows = []) => {
    const roleMap = {
    ...
    return rows.reduce(
      (
        acc,
        { rol_id, aprobado, comentario, usuario_modificacion, fecha_aprobacion }
      ) => {
  ...
  const getProductState = useCallback((approvalDate, approvalList) => {
   ...
   */
  const getIdActiveConvocationOfSupplier = useCallback(() => {
  ...
  const getPlans = useCallback(async () => {
    try {
      setLoading(true);
      const { data, status } =
        await convocationProductsServices.getPlansByConvocation(
  ...
  const normalizeRows = useCallback(
    ...
    [extractObservations, getProductState]
  );

  const getProductList = useCallback(
    async (jornadaPlanId, onlyMe) => {
      try {
        setLoadingTable(true);
        const { data, status } =
          await convocationProductsServices.getProductsBySupplier(
            userAuth.id,
            jornadaPlanId,
            onlyMe
          );
  ...
  const handleSaveProducts = async () => {
    try {
      ...
      const { status } =
        await convocationProductsServices.saveProductBySupplier(sendData);
